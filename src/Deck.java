...
public class Deck {
  ...
  public Deck() {   
    ...
    for (int i = 0; i < 4; i++) { //with a nested loop, we go through the 52 cards.
      for (int j = 1; j < 13; j++) {                                                  
        if (j == 1) { //if j is 0, this is an ace and we will give ace the value of 11 (look at the rules of Blackjack for further information) at the beginning.
          ...
          Card card = new Card(i, j, j); //when j is 1 (for example), we have a two of a suit and this has a value of two. So for the value, we increment j by one.
          deck.add(card); //we add the card to our deck.
        } 
      }
    }
